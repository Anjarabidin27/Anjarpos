...
export const CartView = () => {
  ...
  const handleCheckout = async () => {
    ...
    if (receipt) {
      toast({
        ...
        description: `Invoice ${receipt.id.slice(0, 8)} telah dibuat`,
      });
      clearCart();
  ...
  const handlePrintReceipt = async () => {
    ...
    if (receipt) {
      toast({
        title: "Transaksi Berhasil",
        description: `Invoice ${receipt.id.slice(0, 8)} berhasil dicetak`,
      });
    ...
    if (receipt) {
      try {
        ...
        if (success) {
          toast({
            title: "Berhasil",
            description: `Invoice ${receipt.id.slice(0, 8)} berhasil dicetak ke thermal printer!`,
          });
          clearCart();
          navigate('/');
        } else {
                       ...
                       receipts.slice(-10).reverse().map((receipt) => (
                         ...
                         >
                           <div className="flex items-center justify-between mb-2">
                             <div className="font-medium text-sm">#{receipt.id.slice(0, 8)}</div>
